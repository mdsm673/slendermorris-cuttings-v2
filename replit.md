...
- **Email ILIV Feature** (August 4, 2025):
  - Added gold "Email ILIV" button next to status dropdown for records with fabric selections
  - Modal popup with editable To/Subject/Body fields for sending fabric requests to ILIV supplier
  - Emails sent to 4 recipients: orders@slendermorris.com, slendermorris@gmail.com, export@iliv.co.uk, jurijs_peremots@smd-textiles.co.uk
  - Tracks email status with gold "✓ Emailed ILIV" badge displayed after successful send
  - Database columns added: iliv_email_sent (boolean) and iliv_email_sent_date (timestamp)
